 ...
 * 
 * 使用方法：
 * 1. 确保后端服务已启动：bun run src/02-intermediate/05-file-upload.ts
 * 2. 将此组件复制到 React 项目中
...
function FileUploadTest() {
  ...
  const handleBatchUpload = async () => {
    ...
    try {
      const res = await fetch('http://localhost:3007/upload/batch', {
        method: 'POST',
        body: formData
      })
      const data = await res.json()
  ...
  const fetchFileList = async () => {
    try {
      const res = await fetch('http://localhost:3007/files')
      const data = await res.json()
      if (data.success) {
  ...
  const handleDelete = async (fileId: string, filename: string) => {
    ...
    try {
      const res = await fetch(`http://localhost:3007/files/${fileId}`, {
        method: 'DELETE'
      })
      const data = await res.json()
      ...
      <div className="max-w-6xl mx-auto px-4">
            ...
            {previewUrl && (
              <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                ...
                <img
                  src={previewUrl}
                  alt="预览"
                  className="max-w-sm max-h-64 rounded-lg shadow-sm"
                />
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-4">
              ...
              <button
                ...
                className={`px-6 py-2 rounded-lg transition-colors ${
                  selectedBatchFiles.length === 0 || loading
                    ? 'bg-gray-300 cursor-not-allowed'
                    : 'bg-green-600 text-white hover:bg-green-700'
                }`}
              >
            ...
            {selectedBatchFiles.length > 0 && (
              <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                <p className="text-sm font-medium text-gray-700 mb-2">
                  已选择 {selectedBatchFiles.length} 个文件：
                </p>
        ...
        {uploadResult && (
          <div className={`rounded-lg p-4 mb-6 ${
            ...
            {uploadResult.data && 'uploaded' in uploadResult.data && (
              <p className="text-sm text-green-800 mt-2">
                成功上传 {uploadResult.data.uploaded.length} 个文件
              </p>
            )}
          </div>
        )}
              ...
              {fileList.map((file) => (
                ...
                >
                  ...
                  <div className="aspect-square bg-gray-100 rounded-lg mb-3 overflow-hidden">
                    <img
                      src={`http://localhost:3007${file.url}`}
                  ...
                  <div className="mt-3 flex gap-2">
                    <a
                      href={`http://localhost:3007${file.url}`}
                      target="_blank"
